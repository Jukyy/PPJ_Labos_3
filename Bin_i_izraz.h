...
povratni_tip Bin_i_izraz::produkcija(Node produkcija, Tablica_djelokruga &tablica, std::string ntip){
    //cout << "Bin_i_izraz" << endl;
	if (produkcija.nodes.size() == 1) return produkcija1(produkcija, tablica);
	else return produkcija2(produkcija, tablica);
}

povratni_tip Bin_i_izraz::produkcija1(Node produkcija, Tablica_djelokruga &tablica, std::string ntip){
    povratni_tip izvedbena_vrijednost;
    Node node = produkcija.nodes[0];
	
	Jednakosni_izraz izraz;
	izvedbena_vrijednost = izraz.produkcija(node, tablica);
	
	return izvedbena_vrijednost;
}

povratni_tip Bin_i_izraz::produkcija2(Node produkcija, Tablica_djelokruga &tablica, std::string ntip){
	povratni_tip izvedbena_vrijednost;
